...
\begin{document}
	...
	The direct one-step-ahead Ridge ARX model delivered the best held-out performance. However, the improvement over the simple persistence baseline is modest, which suggests that most short-term predictability comes from recent PM2.5 history. Weather and traffic add incremental signal, but they do not transform forecast accuracy. The predictions from this model can be seen compared to test values in Figure \ref{figure:1}.
	...
	\begin{figure}[h]
		\caption{Daily PM2.5 - Direct one-step-ahead Ridge Arx with Predictions}
		\label{figure:1}
		\centering
		\includegraphics[width=0.955\textwidth]{ridgeARX_pred.png}
	\end{figure}
